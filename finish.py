...
def convert_pdf_to_images(pdf_path: str, dpi: int = 250) -> List[str]:
    ...
    for page_num in range(len(doc)):  # 각 페이지를 순회
        ...
        image_path = os.path.join(output_folder, f"{base_name}_page_{page_num + 1}.png")  # 문서이름+페이지로 저장
...
def main():
    st.set_page_config("우정사업본부 통합 지식 서비스", layout="wide")

    left_column, right_column = st.columns([1, 1])
    with left_column:
        st.header("우정사업본부 통합 지식 서비스")
